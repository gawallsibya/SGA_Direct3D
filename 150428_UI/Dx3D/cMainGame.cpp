...
void cMainGame::Setup()
{
	...
	pImageView->SetImageName("./UI/panel-info.png.png");
	pImageView->SetPosition(D3DXVECTOR3(100, 0, 0));
	...
	pTextView->SetText("동대문 역사 문화 공원으로 가서 김효상 10마리를 잡아오세요.");
	pTextView->SetPosition(D3DXVECTOR3(100, 100, 0));
	pTextView->SetSize(ST_SIZEF(312, 200));
	pTextView->SetDrawTextType(DT_CENTER | DT_VCENTER | DT_WORDBREAK);
	pTextView->SetTextColor(D3DCOLOR_XRGB(230, 130, 0));
	pTextView->SetTag(3);
	...
	pButton->SetNormalImage("./UI/btn-med-up.png.png");
	pButton->SetMouseOverImage("./UI/btn-med-over.png.png");
	pButton->SetSelectedImage("./UI/btn-med-down.png.png");
...
void cMainGame::OnClick( cUIButton* pSender )
{
	if(m_pUIRoot)
	{
		cUITextView* pTextView = (cUITextView*)m_pUIRoot->FindByTag(3);
		if(pTextView)
		{
			pTextView->SetText("확인 버튼 눌렸음.");
		}
	}
}
